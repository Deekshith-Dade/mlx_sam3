        ...
        if self.self_attn is not None:
            ...
            if presence_token is not None:
                ...
                tgt_query_pos_o2o = mx.concat([
                    mx.zeros_like(presence_token), tgt_query_pos
                ], axis = 0)
            ...
            tgt2 = self.self_attn(q, k, tgt_o2o.transpose(1, 0, 2), attn_mask=self_attn_mask).transpose(1, 0, 2)
...
class TransformerDecoder(nn.Module):
    def __init__(
        ...
        if box_refine:
            ...
            self.bbox_embed.layers[-1].weight = init_fn(self.bbox_embed.layers[-1].weight)
        ...
        if self.instance_query_embed is not None:
            self.instance_query_embed.weight = init_normal(self.instance_query_embed.weight)
        ...
        self.compiled_mode = compile_mode
        self.compiled = False
    ...
    def _get_rpb_matrix(self, reference_boxes, feat_size):
        ...
        deltas_y = coords_h.reshape(1, -1, 1) - boxes_xyxy.reshape(-1, 1, 4)[:, :, 1:4:2]
        deltas_y = deltas_y.reshape(bs, num_queries, -1, 2)
        deltas_x = coords_w.reshape(1, -1, 1) - boxes_xyxy.reshape(-1, 1, 4)[:, :, 0:3:2]
    ...
    def __call__(
        ...
        if self.presence_token is not None and is_instance_prompt is False:
            presence_out = mx.broadcast_to(self.presence_token.weight[None], (1, bs, self.d_model))
